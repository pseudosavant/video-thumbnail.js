...
async function basicExample() {
  ...
  try {
    ...
    thumbnails.forEach((thumbnail: ThumbnailResult) => {
...
async function advancedExample() {
  const result = await videoThumbnail('video.mp4', {
    timestamps: [10, 50, 100], // Absolute second timestamps
    size: 1280,
  ...
  // Memory management
  const usage: MemoryUsage = videoThumbnail.getMemoryUsage();
...
// Example 4: Raw Blob output for caller-managed caches
async function blobOutputExample() {
  const thumbnails = await videoThumbnail('video.mp4', {
    timestamps: [0.25],
    size: 640,
    mime: { type: 'image/webp', quality: 0.8 },
    type: 'blob',
    cache: false
  });

  const blob: Blob | undefined = thumbnails[0].blob;
  if (!blob) throw new Error('Expected thumbnail Blob output');
...
// Example 6: Type validation at compile time
function typeValidationExamples() {
  // These would cause TypeScript compilation errors:
  
  // ❌ Invalid MIME type
  // const badOptions1: Options = {
  //   mime: { type: 'image/gif' } // Error: not assignable to ImageMimeType
  // };
  
  // ❌ Invalid output type
  // const badOptions2: Options = {
  //   type: 'base64' // Error: not assignable to OutputType
  // };
  
  // ❌ Invalid quality value
  // const badOptions3: Options = {
